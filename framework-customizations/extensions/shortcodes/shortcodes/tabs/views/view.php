        ...
        foreach ( $atts['tabs'] as $tab ) : ?>
            <li <?php echo ($counter == 1) ? 'class="active"' : '';?>>
                <a href="#<?php echo  $id.'-'.$counter;?>" data-toggle="tab">
                    ...
                    <?php endif;?>
                    <?php echo clemo_theme_translate(esc_attr($tab['tab_title'])); ?>
                </a>
            </li>
				<?php $counter++;
			endforeach; ?>
        ...
        foreach ( $atts['tabs'] as $tab ) : ?>
            <div id="<?php echo  $id.'-'.$cnt;?>" class="tab-pane fade <?php echo ($cnt == 1) ? 'in active' : '';?>">
                <?php if(!empty($tab['tab_content_title'])):?>
                    <strong><?php echo clemo_theme_translate(esc_attr($tab['tab_content_title'])); ?></strong>
                <?php endif;?>

                <?php if(!empty($tab['tab_content'])):?>
                    <?php echo clemo_theme_translate(esc_textarea(do_shortcode($tab['tab_content']) ));?>
                <?php endif;?>
            </div>
        <?php $cnt++; endforeach; ?>
    </div>
