    ...
    <?php if (!empty($atts['list_items'])) : ?>
            <?php foreach ($atts['list_items'] as $list_item) : ?>
                ...
                <li><i class="<?php echo  $atts['list_type']['list-icon']['icon']; ?>"></i>
                    <?php
                    if ($list_item['link'] != '') {
                        echo '<a href="' . esc_url($list_item['link']) . '" target="' . esc_attr($list_item['target']) . '">' . clemo_theme_translate(esc_attr($list_item['item'])) . '</a>';
                    } else {
                        echo clemo_theme_translate(esc_attr($list_item['item']));
                    }
                    ?>
                        ...
                        <?php foreach ($list_item['sublist_items'] as $sublist_item) : ?>
                            ...
                            <li><i class="<?php echo  $atts['list_type']['list-icon']['icon']; ?>"></i>
                                ...
                                if ($sublist_item['link'] != '') {
                                    echo '<a href="' . esc_url($sublist_item['link']) . '" target="' . esc_attr($sublist_item['target_subitem']) . '">' . clemo_theme_translate(esc_attr($sublist_item['subitem'])) . '</a>';
                                } else {
                                    echo clemo_theme_translate(esc_attr($sublist_item['subitem']));
                                }
                                ?>
                            </li>
                            <?php else : ?>
