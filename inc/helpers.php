...
function clemo_editor_data($value) {
...
function clemo_get_option($k, $v = '', $m = 'theme-settings') {
    if (defined('FW')) {
        switch ($m) {
            case 'theme-settings':
...
function clemo_get_image($k, $v = '', $d = false) {

    if ($d == true) {
        $attachment = $k;
    } else {
        $attachment = clemo_get_option($k);
    }
...
function clemo_main($id, $name = true) {
...
// since 2.0
function clemo_sectionID($id, $returnID = false) {
...
function clemo_get_post_meta($id, $needle) {
    $data = get_post_meta($id, 'fw_options');
    if (is_array($data) && isset($data[0]['page_sections'])) {
        $data = $data[0]['page_sections'];

        if (is_array($data)) {
            return clemo_seekKey($data, $needle);
        }
    }
}

function clemo_seekKey($haystack, $needle) {
    foreach ($haystack as $key => $value) {

        if ($key == $needle) {
            return $value;
        } elseif (is_array($value)) {
            return clemo_seekKey($value, $needle);
        }
    }
}
...
if (!function_exists('clemo_theme_button_class')) :
...
if (!function_exists('clemo_theme_translate')) :

    function clemo_theme_translate($content) {
        /**
...
// term
if (!function_exists('clemo_get_category_term_list')) :

    function clemo_get_category_term_list() {
        /**
        ...
        $result = array();
        $result[0] = __('All Categories', 'clemo');
...
if (!function_exists('clemo_get_header_icons')) {

    function clemo_get_header_icons() {
        ...
        $meta_tags = array(
            sprintf('<link rel="icon" href="%s" sizes="32x32" />', esc_url(clemo_get_image('favicon', CLEMO_IMAGES . '/ico/favicon.png'))),
            sprintf('<link rel="icon" href="%s" sizes="192x192" />', esc_url(clemo_get_image('site_icon_192', CLEMO_IMAGES . '/ico/site-icon-192.png'))),
            sprintf('<link rel="apple-touch-icon-precomposed" href="%s" />', esc_url(clemo_get_image('site_icon_180', CLEMO_IMAGES . '/ico/site-icon-180.png'))),
            sprintf('<meta name="msapplication-TileImage" content="%s" />', esc_url(clemo_get_image('site_icon_270', CLEMO_IMAGES . '/ico/site-icon-270.png'))),
        );
...
function clemo_edit_section() {
    ?>
    <div class="section-edit">
        <div class="container relative">
    <?php
    if (is_user_logged_in()) {
        edit_post_link(esc_html__('Edit', 'clemo'), '', '');
    }
    ?>
